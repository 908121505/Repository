package com.honglu.quickcall.common.third.newrongyun.io.rong.methods.message.chatroom;
...
/**
 * 发送聊天室消息方法 docs :
 * http://www.rongcloud.cn/docs/server.html#message_chatroom_publish
 * 
 * @author RongCloud
 *
 */
public class Chatroom {
	 ...
	 *            objectName
	 *            为自定义消息类型，该参数可自定义格式。融云消息类型在messages下，自定义消息继承BaseMessage即可（必传）
	 *
	...
	public ResponseResult broadcast(ChatroomMessage message) throws Exception {
		...
		HttpURLConnection conn = HttpUtil.CreatePostHttpConnection(rongCloud.getApiHostType(), appKey, appSecret,
				"/message/chatroom/broadcast.json", "application/x-www-form-urlencoded");
		HttpUtil.setBodyParameter(body, conn);
