package com.honglu.quickcall.common.third.newrongyun.io.rong.methods.sensitive;
...
import com.honglu.quickcall.common.third.newrongyun.io.rong.models.response.ListWordfilterResult;
...
public class SensitiveWord {
	...
	public ResponseResult add(SensitiveWordModel sensitiveword) throws Exception {
		...
		HttpURLConnection conn = HttpUtil.CreatePostHttpConnection(rongCloud.getApiHostType(), appKey, appSecret,
				"/sensitiveword/add.json", "application/x-www-form-urlencoded");
	 ...
	 * 查询敏感词列表方法
	 * 
	 * @param type:查询敏感词的类型，0
	 *            为查询替换敏感词，1 为查询屏蔽敏感词，2 为查询全部敏感词。默认为 1。（非必传）
	 *
	...
	public ListWordfilterResult getList(Integer type) throws Exception {
		...
		HttpURLConnection conn = HttpUtil.CreatePostHttpConnection(rongCloud.getApiHostType(), appKey, appSecret,
				"/sensitiveword/list.json", "application/x-www-form-urlencoded");
		HttpUtil.setBodyParameter(body, conn);

		return (ListWordfilterResult) GsonUtil.fromJson(
				CommonUtil.getResponseByCode(PATH, CheckMethod.GETLIST, HttpUtil.returnResult(conn)),
				ListWordfilterResult.class);
	}
	...
	public ResponseResult remove(String word) throws Exception {
		...
		HttpURLConnection conn = HttpUtil.CreatePostHttpConnection(rongCloud.getApiHostType(), appKey, appSecret,
				"/sensitiveword/delete.json", "application/x-www-form-urlencoded");
		HttpUtil.setBodyParameter(body, conn);
	...
	public ResponseResult batchDelete(String[] words) throws Exception {
		...
		HttpURLConnection conn = HttpUtil.CreatePostHttpConnection(rongCloud.getApiHostType(), appKey, appSecret,
				"/sensitiveword/batch/delete.json", "application/x-www-form-urlencoded");
		HttpUtil.setBodyParameter(body, conn);

		return (ResponseResult) GsonUtil.fromJson(
				CommonUtil.getResponseByCode(PATH, CheckMethod.BATCH_DELETE, HttpUtil.returnResult(conn)),
				ResponseResult.class);
	}
