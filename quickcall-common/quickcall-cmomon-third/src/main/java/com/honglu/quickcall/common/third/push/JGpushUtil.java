...
public class JGpushUtil {
	protected static final Logger logger = LoggerFactory.getLogger(JGpushUtil.class);

	// 加载配置信息
	private static final String appKey = ResourceBundle.getBundle("thirdconfig").getString("JPush_key");;
	private static final String masterSecret = ResourceBundle.getBundle("thirdconfig").getString("JPush_masterSecret");

	public static void main(String[] args) {
		JGpushUtil.pushMessageWithType("nvhsn", "新增粉丝，用户：" + "测试测试" + "关注了你。", 4, "126", "103");
	}

	public static void pushMessageWithType(String title, String content, Integer type, String aliasses,
			String jumpParam) {
	 ...
	 * @return
	 */
	public static PushPayload pushMessageWithTypeToObject(String title, String content, Integer type, String aliasses,
			String jumpParam) {
		...
		return PushPayload.newBuilder().setPlatform(Platform.android_ios()).setAudience(Audience.alias(strs))
				.setNotification(Notification.newBuilder()
						// 推送到Android
						.addPlatformNotification(AndroidNotification.newBuilder().setTitle(title).setAlert(content)
								.addExtra("from", "JPush").addExtra("type", type).addExtra("jumpParam", jumpParam)
								.build())
						// 推送到IOS
						.addPlatformNotification(IosNotification.newBuilder().setAlert(content)
								.addExtra("from", "JPush").setBadge(5).setSound("happy").addExtra("type", type)
								.addExtra("jumpParam", jumpParam).build())
						.build())
