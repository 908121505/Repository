...
public class JsonUtil {
	...
	public static <T> T getJsonObject(String path, String jsonName) throws IOException {
		...
		try {
			...
			/*
			 * if(JSONFILE.contains(".jar")){ reader =new BufferedReader( new
			 * InputStreamReader(loadResourceFromJarURL(JSONFILE,path,jsonName))); }else{
			 * reader =new BufferedReader( new InputStreamReader(new
			 * FileInputStream(JSONFILE+path+jsonName))); }
			 */
			reader = new BufferedReader(
					new InputStreamReader(JsonUtil.class.getResourceAsStream("/jsonsource/" + path + jsonName)));
