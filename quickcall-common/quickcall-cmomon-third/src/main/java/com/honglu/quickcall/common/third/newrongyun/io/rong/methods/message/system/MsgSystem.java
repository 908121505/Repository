package com.honglu.quickcall.common.third.newrongyun.io.rong.methods.message.system;
...
import com.honglu.quickcall.common.third.newrongyun.io.rong.models.Templates;
import com.honglu.quickcall.common.third.newrongyun.io.rong.models.message.BroadcastMessage;
import com.honglu.quickcall.common.third.newrongyun.io.rong.models.message.MessageModel;
...
public class MsgSystem {
	...
	/**
	 * 发送系统消息方法（一个用户向一个或多个用户发送系统消息，单条消息最大 128k，会话类型为 SYSTEM。 每秒钟最多发送 100 条消息，每次最多同时向
	 * 100 人发送，如：一次发送 100 人时，示为 100 条消息。）
	 *
	 * @param message
	 *            消息体
	...
	public ResponseResult send(MessageModel message) throws Exception {
		...
		return (ResponseResult) GsonUtil.fromJson(
				CommonUtil.getResponseByCode(PATH, CheckMethod.PUBLISH,
						CommonUtil.getResponseByCode(PATH, CheckMethod.PUBLISH, HttpUtil.returnResult(conn))),
	...
	public ResponseResult sendTemplate(TemplateMessage template) throws Exception {
		...
		return (ResponseResult) GsonUtil.fromJson(
				CommonUtil.getResponseByCode(PATH, CheckMethod.PUBLISHTEMPLATE, HttpUtil.returnResult(conn)),
				ResponseResult.class);
	}

	/**
	 * 发送广播消息方法（发送消息给一个应用下的所有注册用户，如用户未在线会对满足条件（绑定手机终端）的用户发送 Push 信息，单条消息最大
	 * 128k，会话类型为 SYSTEM。每小时只能发送 2 次，每天最多发送 3 次。）
	 * 该功能开发环境下可免费使用。生产环境下，您需要登录开发者后台，在“应用/IM
	 * 服务/高级功能设置”中开通公有云专业版后，在“广播消息和推送”中，开启后才能使用
	 *
	 * @param message
	 *            消息体
	 *
