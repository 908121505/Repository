...
public class CertificationController {
	...
	public WebResponseModel skillAuditUpload(HttpServletRequest request) {
		...
		/*
		 * if ("000000".equals(response.getCode())) { SaveSkillAuditRequest params = new
		 * SaveSkillAuditRequest(); params.setCustomerId(Long.valueOf(customerId));
		 * params.setSkillItemId(Long.valueOf(skillItemId));
		 * params.setSkillVoiceTime(new BigDecimal(skillVoiceTime).setScale(1,
		 * BigDecimal.ROUND_HALF_UP)); params.setSkillVoiceUrl(response.getData()); //
		 * 调用保存技能认证服务 response = userCenterService.execute(params); }
		 * logger.info("userWeb.certification skillAuditUpload response data : " +
		 * response);
		 */
		response.setData(response.getData());
		return response;
