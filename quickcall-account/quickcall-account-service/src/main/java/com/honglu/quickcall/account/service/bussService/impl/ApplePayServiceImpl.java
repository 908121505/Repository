...
import com.honglu.quickcall.account.facade.enums.RechargeStateEnum;
import com.honglu.quickcall.account.facade.enums.RechargeTypeEnum;
...
public class ApplePayServiceImpl implements ApplePayService {
	...
	public CommonResponse createOrder(ApplePayRequest applePayRequest) {
		...
		try {
			// 参数校验
			// Map<String, String> validMap = this.validateRequestParams(appleOrderRequest,
			// headerBo.getSign());
			// LOGGER.info("ApplePayBizImpl.createOrder，创建苹果预支付订单参数校验结果validMap：{}",
			// JSONObject.toJSONString(validMap, SerializerFeature.WriteMapNullValue));
			...
			// Long setNx = RedisUtils.setNx(SysParamsCons.GM_PAYAPI_CP_ORDER_ID + ":" +
			// appleOrderRequest.getBizData().getCpOrderId(),
			// SysConstants.CP_ORDER_ID_EXPIRE_TIME,
			// appleOrderRequest.getBizData().getCpOrderId());
			// LOGGER.info("ApplePayBizImpl.createOrder setNx ：{}", setNx);
			//
			// //session校验
			// boolean sessionFlag =
			// this.rpcInvokeService.checkSession(appleOrderRequest.getUserId(),
			// appleOrderRequest.getSessionId(), appleOrderRequest.getAppId());
			// LOGGER.info("ApplePayBizImpl.createOrder，创建苹果预支付订单session校验是否通过：{}",
			// sessionFlag);
			// if(!sessionFlag) {
			// this.putResultToMap(ResponseCode.USER_SESSION_EXPIRE.getValue(),
			// ResponseCode.USER_SESSION_EXPIRE.getDesc(), resMap);
			// return resMap;
			// }
			// //验签
			// boolean signFlag = this.payCheckSignBiz.checkSign(appleOrderRequest,
			// appleOrderRequest.getBizData(), headerBo.getSign());
			// LOGGER.info("ApplePayBizImpl.createOrder，创建苹果预支付订单参数验签是否通过：{}", signFlag);
			// if(!signFlag) {
			// this.putResultToMap(ResponseCode.ORDER_PAY_SIGN_IS_ERROR.getValue(),
			// ResponseCode.ORDER_PAY_SIGN_IS_ERROR.getDesc(), resMap);
			// return resMap;
			// }
			// //查询appKey
			// MAppVo mAppVo =
			// this.rpcInvokeService.getMAppByAppId(appleOrderRequest.getAppId());
	...
	public CommonResponse applePurchase(ApplePurchaseRequest applePurchaseRequest) {
		...
		try {
			/**
			 * 1.参数校验 2.session校验 3.验签 4.查询充值订单
			 */
			...
			if (states.equals("0")) {
				...
				if (!applePurchaseRequest.getTradeNo().equalsIgnoreCase(transaction_id)) {
					LOGGER.error("单号不一致，请求单号transNo：{}，返回单号transaction_id：{}", applePurchaseRequest.getTradeNo(),
							transaction_id);
					return ResultUtils.result(BizCode.DataNotExist);
				}
				// 充值单去重
				...
				if (RechargeStateEnum.PAY_SUCCESS.getType().equals(rechargeVO.getState())) {
					LOGGER.warn("单号已充值成功，请勿重复操作，请求单号transNo：{}，充值单号：{}", applePurchaseRequest.getTradeNo(),
							applePurchaseRequest.getOrderId());
