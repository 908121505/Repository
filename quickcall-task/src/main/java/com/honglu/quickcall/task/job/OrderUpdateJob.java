...
public class OrderUpdateJob {
	...
	public void updateOrderStatusReceiveOrder() {
		...
		try {
			...
			List<TaskOrder> orderList = taskOrderMapper.queryReceiveOrderOverTime(currTime, endTime, queryStatus,
					updateStatus, skillType, queryEndTime);
			refundToCustomer(orderList, CANCEL_ONE);
			updateOrderStatusByOrderListForCancel(orderList, updateStatus);
			// taskOrderMapper.waittingReceiveOrderOverTime(currTime, endTime, queryStatus,
			// updateStatus, skillType);
	...
	public void updateOrderStatusDvStartService() {
		...
		try {
			...
			// 大V未发起立即服务超时给账户金额
			Date queryEndTime = getEndTimeByAddHours(-1);
			List<TaskOrder> orderList = taskOrderMapper.queryStartOrderOverTime(currTime, endTime, queryStatus,
					updateStatus, skillType, queryEndTime);
			refundToCustomer(orderList, CANCEL_TWO);
			updateOrderStatusByOrderListForCancel(orderList, updateStatus);
			// taskOrderMapper.startOrderOverTime(currTime, endTime, queryStatus,
			// updateStatus, skillType);
	...
	public void updateOrderStatusAppointGoing() {
		...
		try {
			...
			Date queryEndTime = getEndTimeByAddHours(-1);
			List<TaskOrder> orderList = taskOrderMapper.queryAppointOrderGoing(currTime, endTime, queryStatus,
					updateStatus, skillType, queryEndTime);
			updateOrderStatusByOrderListForCancel(orderList, updateStatus);
			// taskOrderMapper.appointOrderGoing(currTime,endTime, queryStatus,
			// updateStatus, skillType,queryEndTime);
		} catch (Exception e) {
	...
	public void updateOrderStatusAfter12HourCust() {
		...
		try {
			...
			// 服务完成，大V金额冻结
			List<TaskOrder> orderList = taskOrderMapper.queryOrderStatusAfter12HourCust(currTime, endTime, queryStatus,
					updateStatus, skillType, queryEndTime);
	...
	public void updateOrderStatusAfter12HourBoth() {
		...
		try {
			...
			Date queryEndTime = getEndTimeByAddDays(-2);
			// 服务完成，大V金额冻结
			List<TaskOrder> orderList = taskOrderMapper.queryOrderStatusAfter12HourBoth(currTime, endTime, queryStatus,
					updateStatus, skillType, queryEndTime);
			freezeToService(orderList);
			updateOrderStatusByOrderListForFinish(orderList, updateStatus);
			sendMsgByOrderList(orderList, CANCEL_FOUR);
	 ...
	 * @param serviceId
	 * @param cancelType
	 *            1:15分钟未接受订单 2:大V5分钟未发起立即服务 3：用户未接受大V立即服务 4：订单自动完成
	 * @param msgContent
	 */
	 ...
	 * @param order
	 * @param evaluateStars
	 * @return @计算公式：完成一笔价值评价=[（log(100,该技能累计订单数+6))*10*平台笔数权重+笔单价*平台笔单价权重]*(评价*评价权重*平台价值权重)
	...
	private BigDecimal calculateOrderSkillScore(Order order, Integer evaluateStars) {
		...
		// 计算技能总比价得分
		BigDecimal orderTotalScore = new BigDecimal(
				(2 / Math.log10(orderTotal + 6)) * 10 * ScoreRankConstants.PLATFORM_ORDER_NUM_TOTAL_WEIGHT);

		// 计算技能笔单价得分
		BigDecimal servicePriceScore = order.getServicePrice()
				.multiply(new BigDecimal(ScoreRankConstants.PLATFORM_SINGLE_ORDER_PRICE_WEIGHT));

		// 计算总得分
		BigDecimal valueScore = orderTotalScore.add(servicePriceScore)
				.multiply(calculateEvaluateScore(evaluateStars, order.getOrderNum()));
